...
export function handleTransfer(event: Transfer): void {
  ...
  if (from.toHexString() != ADDRESS_ZERO && from.toHexString() != pair.id) {
    ...
    let result = pairContract.try_balanceOf(from)
    if (!result.reverted) {
      fromUserLiquidityPosition.liquidityTokenBalance = convertTokenToDecimal(result.value, BI_18)
  ...
  if (event.params.to.toHexString() != ADDRESS_ZERO && to.toHexString() != pair.id) {
    ...
    let result = pairContract.try_balanceOf(to)
    if (!result.reverted) {
      toUserLiquidityPosition.liquidityTokenBalance = convertTokenToDecimal(result.value, BI_18)
